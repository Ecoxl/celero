 ...
 *  base_link = `Ostim.widgets.HTMLPanel <HTMLPanel.html>`_
 */
 ...
 *
 *  Remark:
 *  with pre v0.73. Ostim.widgets.ContextBrowserPanel was renamed to
 *  Ostim.widgets.BookmarksPanel. Existing code can be converted (and
 *  expanded) like this:
 *
 ...
 *  the point (x,y) where the map should be centered.
 */
Ostim.widgets.BookmarksPanel = Ext.extend(Ostim.widgets.HTMLPanel, {
     ...
     *  for compatibility with pre v0.73 - Ostimon.widgets.ContextBrowserPanel or
	...
	initComponent: function () {
		...
		Ostim.widgets.BookmarksPanel.superclass.initComponent.call(this);
				...
				if (contexts[i].id.substr(0, 11) == "hr_bookmark") {
					...
					if (this.showUserBookmarks) {
						...
						if (this.isValidBookmark(contexts[i])) {
							...
							restoreTooltip += "'";
							htmllines += '<div class="hr-bookmark-link-user" style="width: 80%;"><a href="#" id="' + contexts[i].id + '" title="' + restoreTooltip + '" onclick="Ostim.widgets.Bookmarks.setMapContext(\'' + this.id + "','" + contexts[i].id + '\'); return false;">' + contexts[i].name + '</a></div>';
						}
						else {
						...
						htmllines += '<div class="x-tool hr-bookmark-close-icon" title="' + removeTooltip + ' \'' + contexts[i].name + '\'" onclick="Ostim.widgets.Bookmarks.removeBookmark(\'' + this.id + "','" + contexts[i].id + '\')">&nbsp;</div>';
					}
				}
				else {
					...
					if (this.showProjectBookmarks) {
						...
						if (contexts[i].desc.length) {
							htmllines += '<div class="hr-bookmark-link-project"><a href="#" id="' + contexts[i].id + '" title="' + contexts[i].desc + '" onclick="Ostim.widgets.Bookmarks.setMapContext(\'' + this.id + "','" + contexts[i].id + '\'); return false;">' + contexts[i].name + '</a></div>';
						} else {
	...
	addBookmark: function () {
		...
		//Add the Ostim-bookmark to localStorage
		localStorage.setItem(this.scId, newbookmarkJSON);
		//Increase number of Ostim-bookmarks
		localStorage.setItem("hr_bookmarkMax", bookmarkmaxNr);

		//Add the Ostim-bookmark to hropts
	...
	removeBookmark: function (id) {
		...
		//If this is the last bookmark, decrease max. number of Ostim-bookmarks
		var strBookmarkMaxNr = localStorage.getItem("hr_bookmarkMax")
		var bookmarkmaxNr = Number(strBookmarkMaxNr)
...
Ext.reg('hr_bookmarkspanel', Ostim.widgets.BookmarksPanel);

// For compatibility with pre v0.73. Ostim.widgets.ContextBrowserPanel was renamed to Ostim.widgets.BookmarksPanel
Ext.reg('hr_contextbrowserpanel', Ostim.widgets.BookmarksPanel);
