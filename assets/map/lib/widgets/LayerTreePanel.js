 ...
 */
Ostim.widgets.LayerTreePanel = Ext.extend(Ext.tree.TreePanel, {
     ...
     *  Use the Ostim Map Context (HMC) provided by Ostim.App. The HMC defines a LayerTree. Default value is false.
     */
    useMapContext: false,
     ...
     *  Context menu (right-click) for layer nodes, for now instance of Ostim.widgets.LayerNodeContextMenu. Default value is null.
     */
    contextMenu: null,
    ...
    initComponent: function () {
        ...
        if (this.useMapContext) {
            var context = Ostim.App.getMapContext();
...
//		}
        ...
        if (this.contextMenu) {
            ...
            this.contextMenu = new Ostim.widgets.LayerNodeContextMenu(cmArgs);
        }

        Ext.apply(this, options);
        Ostim.widgets.LayerTreePanel.superclass.initComponent.call(this);
    ...
    createNode: function (treeLoader, attr) {
        ...
        var mapPanel = Ostim.App.getMapPanel();
    ...
    onExpandNode: function (node) {
        for (var i = 0; i < node.childNodes.length; i++) {
            ...
            if (child.leaf) {
                this.setNodeEnabling(child, Ostim.App.getMap());
            }
        }
    },
    ...
    getLayerNrInTree: function (layerName) {
        var treePanel = Ostim.App.topComponent.findByType('hr_layertreepanel')[0];
        this.intLayer = -1;
        var blnFound = false;
        if (treePanel != null) {
...
Ext.reg('hr_layertreepanel', Ostim.widgets.LayerTreePanel);
