 ...
 *	  "hr_multilayer".
 */
Ostim.widgets.MultiLayerNode = Ext.extend(GeoExt.tree.LayerNode, {
	...
	constructor: function(config) {
		...
		Ostim.widgets.MultiLayerNode.superclass.constructor.apply(this, arguments);
	},
	...
	render: function(bulkRender) {
		...
		Ostim.widgets.MultiLayerNode.superclass.render.apply(this, arguments);
	},
	...
	onLayerVisibilityChanged: function() {
		// One-time rendering needed only
		this.layer = this.layers[0];
		Ostim.widgets.MultiLayerNode.superclass.onLayerVisibilityChanged.apply(this, arguments);
	},

	/** private: method[onCheckChange]
	 *  :param node: ``Ostim.widgets.MultiLayerNode``
	...
	onCheckChange: function(node, checked) {
		...
		for (var i = 0; i < this.layers.length; i++) {
			...
			Ostim.widgets.MultiLayerNode.superclass.onCheckChange.apply(this, arguments);
		}
	},
	...
	onStoreAdd: function(store, records, index) {
		...
		for (var i = 0; i < this.layers.length; i++) {
			this.layer = this.layers[i];
			Ostim.widgets.MultiLayerNode.superclass.onStoreAdd.apply(this, arguments);
		}
	},
	...
	onStoreRemove: function(store, record, index) {
		...
		for (var i = 0; i < this.layers.length; i++) {
			this.layer = this.layers[i];
			Ostim.widgets.MultiLayerNode.superclass.onStoreRemove.apply(this, arguments);
		}
	},
	...
	onStoreUpdate: function(store, record, operation) {
		...
		for (var i = 0; i < this.layers.length; i++) {
			this.layer = this.layers[i];
			Ostim.widgets.MultiLayerNode.superclass.onStoreUpdate.apply(this, arguments);
		}
	},
	...
	destroy: function() {
		...
		for (var i = 0; i < this.layers.length; i++) {
			this.layer = this.layers[i];
			Ostim.widgets.MultiLayerNode.superclass.destroy.apply(this, arguments);
		}
	}
});
 ...
 */
Ext.tree.TreePanel.nodeTypes.hr_multilayer = Ostim.widgets.MultiLayerNode;
