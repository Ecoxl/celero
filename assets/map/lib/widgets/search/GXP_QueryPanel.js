...
/** api: example
 *
 *  Sample code showing how to configure a Ostim GXP_QueryPanel. Here within a SearchCenterPanel, through a search button
 *  (binoculars) within the MapPanel Toolbar. This Panel is mostly used in combination with the  `Ostim.widgets.search.FeaturePanel <FeaturePanel.html>`_
 *  in which results from a search are displayed in a grid and on the Map. Both Panels are usually bundled
 *  in a `Ostim.widgets.search.SearchCenterPanel <SearchCenterPanel.html>`_ that manages the search and result Panels.
...
Ostim.widgets.GXP_QueryPanel_Empty = Ext.extend(Ext.Panel, {} );
 ...
 *  used in Ostim as well. A GXP QueryPanel allows a user to construct query conditions interactively.
 *  These query conditions will be translated to WFS Filters embedded in an WFS GetFeature request.
 *  Results may be displayed in a  `Ostim.widgets.search.FeaturePanel <FeaturePanel.html>`_,
 *  see example configuration.
 */
Ostim.widgets.search.GXP_QueryPanel = Ext.extend(gxp.QueryPanel, {
...
// See also: http://ian01.geog.psu.edu/geoserver_docs/apps/gaz/search.html
    initComponent: function () {
        var map = this.map = Ostim.App.getMap();
...
//                    }
		...
		if ( config.layerStore.data.items[0] ) {
        	...
        	Ostim.widgets.search.GXP_QueryPanel.superclass.initComponent.call(this);
			...
			Ext.apply(this, {});
			Ostim.widgets.GXP_QueryPanel_Empty.superclass.initComponent.apply(this, arguments);
        }
        ...
        Ext.each(wmsLayers, function (wmsLayer) {
            ...
            if (wfsOpts.protocol === 'fromWMSLayer') {
                ...
                if (protocol.url instanceof Array) {
                    protocol.url = Ostim.Utils.randArrayElm(protocol.url);
...
Ext.reg('hr_gxpquerypanel', Ostim.widgets.search.GXP_QueryPanel);
