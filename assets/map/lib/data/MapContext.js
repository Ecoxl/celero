...
Ostim.data.MapContext = {
    ...
    saveContext: function (mapPanel, options) {
        ...
        var form = Ext.DomHelper.append(
                document.body,
                {
                    ...
                    id: 'hr_downloadForm',
                    method: 'post',
                    /** Ostim CGI URL, see /services/ostim.cgi. */
                    action: Ostim.globals.serviceUrl,
                    children: formFields
                }
        );
     ...
     *  This formatXml differs from Ostim.Utils.formatXml:
     *      less returns (tag/end-tag on one line if only one value in between)
     *  If accepted, replace Ostim.Utils.formatXml with this one
     */
     ...
     *  If accepted, probably better placed in Ostim.Utils
     */
