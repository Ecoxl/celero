 ...
 * @since 02-12-2014
 */
$app->get("/ISPotentialsNew_json_test_by_project", function () use ($app, $db, $pdo) {
    ...
    if(isset($_GET['prjId']) && $_GET['prjId']!="" && $_GET['prjId']!=-1) {
        $projectID = $_GET['prjId'];
 ...
 * @since 24-11-2014
 */
$app->get("/companies_json_test2", function () use ($app, $pdo) {
    //$pdo->exec('SET NAMES "utf8"');
    //$res = $pdo->query(  "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'ecoman_18_08' AND TABLE_NAME = 't_flow';"  )->fetchAll(PDO::FETCH_ASSOC);
 ...
 * @since 11-09-2014
 */
$app->post("/insertISManual", function () use ($app, $db, $pdo) {
    ////$pdo->exec('SET NAMES "utf8"');
    $stmt_log=$pdo->prepare("
        INSERT INTO t_is_prj(synergy_id, consultant_id,  name, status, prj_id) VALUES 
        (:synergy,:consultant_id,:name, :status, :prj_id)");
    $test = $_POST["row"];
    ...
    */
    try { 
                ...
                $id =$stmt_log->execute(array(':synergy'=>$_POST["IS"],
                ...
                foreach ($arrayRows as $row){
                        ...
                        $insertValueStr.= "(".$arrExplode[0].",".$arrExplode[1].","
                                . "".$arrExplode[2].","
                                . "".$arrExplode[3].","
                                . "".$row["qntty1"].","
                                . "".$row["qntty2"].","
...
$app->post("/insertIS", function () use ($app, $db, $pdo) {
    ...
    */
    try { 
                ...
                foreach ($arrayRows as $row){
                        $arrExplode = explode(",", $row["id"]);
                        $insertValueStr.= "(".$arrExplode[0].",".$arrExplode[1].",".$arrExplode[2].",".$arrExplode[2].",".$row["qntty1"].",".$row["qntty2"].",".$insertID."),";
...
$app->get("/ISScenarios", function () use ($app, $db, $pdo) {
    ...
    if(isset($_GET['userID']) && $_GET['userID']!="") {
        $userID_str = ' WHERE prj.consultant_id='.trim($_GET['userID']);
    } else {
    ...
    $res2 = $pdo->query(  "SELECT 
                            ...
                            FROM t_is_prj as prj
                            INNER JOIN t_synergy AS syn ON prj.synergy_id = syn.id
                            ".$userID_str." "  )->fetchAll(PDO::FETCH_ASSOC);
    $flows = array();
    foreach ($res as $flow){
        $flows[]  = array(
            "id" => $flow["prj_id"],
