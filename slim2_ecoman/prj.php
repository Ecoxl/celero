 ...
 * @since 27-01-2015
 */
//$app->get("/getFlowDetailsCns_prj", function () use ($app, $pdo) {  
$app->post("/getFlowDetailsCns_prj", function () use ($app, $pdo) {  
    

    
    if(isset($_POST['items']) && $_POST['items']!="" && isset($_POST['items']) && $_POST['items']!="") {
        $items = $_POST['items'];
        $itemsArr = json_decode(stripslashes($items), true);
        //print_r($itemsArr);
        $sorguStr = "WHERE (sm.cmpny_id=".$itemsArr['from']." OR sm.cmpny_id=".$itemsArr['to'].") AND sm.flow_id=".$itemsArr['flow']." ";
        ...
        dt.cmpny_to_id
          FROM t_is_prj isp
          INNER JOIN t_is_prj_details dt ON isp.id=dt.is_prj_id
          WHERE isp.consultant_id=8
          ;*/

    }
    
        /*print_r('SELECT 
                            sm.potential_energy as potential_energy,
                            sm.potential_energy_unit as potential_energy_unit,
                            sm.supply_cost as supply_cost,
                            sm.supply_cost_unit as supply_cost_unit,
                            sm.transport_id as transport_id,
                            sm.entry_date as entry_date,
                            sm.entry_date as entry_date,
                            sm.concentration as concentration,
                            sm.pression as pression,
                            sm.state_id as state_id,
                            sm.min_flow_rate as min_flow_rate,
                            sm.min_flow_rate_unit as min_flow_rate_unit,
                            sm.max_flow_rate as max_flow_rate,
                            sm.max_flow_rate_unit as max_flow_rate_unit,
                            sm.ep_unit_id as ep_unit_id,
                            sm.qntty_unit_id as qntty_unit_id,
    ...
    foreach ($res as $flow){
        $flows[]  = array(
            ...
            "potential_energy" => $flow["potential_energy"],
            "potential_energy_unit" => $flow["potential_energy_unit"],
            "supply_cost" => $flow["supply_cost"],
            "supply_cost_unit" => $flow["supply_cost_unit"],
            "transport_id" => $flow["transport_id"],
            "entry_date" => $flow["entry_date"],
            "concentration" => $flow["concentration"],
            "pression" => strtolower($flow["pression"]),
            "state_id" => strtolower($flow["state_id"]),
            "min_flow_rate" => strtolower($flow["min_flow_rate"]),
            "min_flow_rate_unit" => strtolower($flow["min_flow_rate_unit"]),
            "max_flow_rate" => strtolower($flow["max_flow_rate"]),
            "max_flow_rate_unit" => strtolower($flow["max_flow_rate_unit"]),
            "ep_unit_id" => strtolower($flow["ep_unit_id"]),
            "comment" => $flow["comment"],
            "description" => $flow["description"],
            "quality" => $flow["quality"],
            "qntty" => $flow["qntty"],
            "qntty_unit_id" => $flow["qntty_unit_id"],
            "substitute_potential" => $flow["substitute_potential"],
            //"link" => '<a href="new_flow/'.$flow["id"].'" onclick="" class="easyui-linkbutton" iconCls="icon-back" plain="true">Dataset Management</a>',
        );
    }
 ...
 * @since 27-01-2015
 */
$app->get("/getFlowDetailsMan_prj", function () use ($app, $pdo) {
 ...
 * @since 27-01-2015
 */
$app->get("/getFlowDetails_prj", function () use ($app, $pdo) {
...
$app->get("/ISPotentialsNew_json_test_by_project_prj", function () use ($app, $db, $pdo) {
    ...
    $res = $pdo->query('SELECT 
                            ...
                            order by flow ASC;
                           /* ORDER BY  '.$sort.' '.$order.'*/  ')->fetchAll(PDO::FETCH_ASSOC);
 ...
 * @since 24-11-2014
 */
$app->get("/companies_json_test2_prj", function () use ($app, $pdo) {
    //$pdo->exec('SET NAMES "utf8"');
    //$res = $pdo->query(  "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'ecoman_18_08' AND TABLE_NAME = 't_flow';"  )->fetchAll(PDO::FETCH_ASSOC);
    ...
    /*print_r('select count(*) as toplam FROM t_flow_total_per_cmpny
                            --INNER JOIN t_cmpny as cmp ON cmp.id = t_flow_total_per_cmpny.cmpny_id
                            '.$sorguStr.'
                            ;');*/
...
$app->get("/columnflows_json_test", function () use ($app, $db, $pdo, $json_str) {
    ...
    foreach ($res as $r){
        //$columnArray['field'] = strtolower($r['name']);
        $columnArray['field'] = strtolower(trim($r['name']));
        $columnArray['title'] = strtolower($r['name']);
        //$columnArray['field'] = 'test';
        //$columnArray['title'] = 'test';
        $columnArray['width'] = 80;
        $columnArray['sortable'] = true;
